...
export default {
    ...
    methods: {
        ...
        handleRemoveCombo({ kitId, totalPrice }) {
            const products = this.items.filter(item => item.kit_id === parseInt(kitId));
        ...
        async remove({ item, items, totalPrice }) {
            const cartItem = item || items[0];

            if (this.loading[cartItem.id]) {
                return;
            }

            this.setLoading(cartItem.id, true);

            await this.removeItem({ item, items, totalPrice });

            this.setLoading(cartItem.id, false);
        },
